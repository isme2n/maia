...
- Part 4 UX closeout baseline is now locked for OSS prep: `doctor` remains infrastructure-first first-run guidance, README/help/tests are aligned to interactive `maia agent new`, and operator-facing command wording is simplified. Recorded closeout evidence: `python3 -m maia --help`, `python3 -m maia doctor --help`, `python3 -m maia setup --help`, `python3 -m maia agent --help`, and `python3 -m pytest -q tests/test_cli.py tests/test_keryx_models.py tests/test_keryx_storage.py tests/test_keryx_server.py` (`92 passed`).
- Part 5 open-source polish closeout is complete via Tasks 143A/143B/143C: the public-surface contract matrix and drift checks were added, README was restructured around product/install/quickstart/concepts, and contributor-facing guidance was split into `CONTRIBUTING.md`, `TESTING.md`, and `ARCHITECTURE.md` with README entry links. Recorded closeout evidence: `python3 -m maia --help`, `python3 -m maia agent --help`, and `python3 -m pytest -q tests/test_cli.py tests/test_cli_runtime.py tests/test_keryx_models.py tests/test_keryx_storage.py tests/test_keryx_server.py` (`151 passed`).
